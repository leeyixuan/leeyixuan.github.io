...
注意：
1. checkout只是为了更新工作区的内容，整个过程只会改变HEAD的指向，不会修改任何分支的指向。
2. checkout不仅仅可以在不同分支之间来回切换，而且还能在指定某个标签或者版本号，在历史版本之间互相切换，十分灵活。
3. 在不同分支之间来回切换时，只存在于老的提交节点中的文件会被删除，新提交节点中的所有文件都会被复制到暂存区域和工作目录中。
4. 如果命令中仅仅指定文件名而没有指定提交节点（分支名、版本号、标签），则会从暂存区域中拷贝内容。
...
注意：
1. reset默认仅仅修改缓存区的内容，可以通过使用`--hard`抛弃当前工作区的修改同时更新工作区，使用`--soft`选项只改变HEAD的指向。
2. 如果没有给出提交点的版本号，那么默认用HEAD，那么缓存区被最后一次版本库覆盖。
